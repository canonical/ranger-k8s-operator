...
help:
	...
	@echo "  install-uv        Install uv (required for venv and tox)"
...
.PHONY: install-uv
install-uv:
	@which uv >/dev/null || (sudo snap install astral-uv --channel latest/stable --classic && uv tool update-shell)
	@echo "NOTE: Run 'source ~/.bashrc' (or open a new shell) if uv was just installed."

.PHONY: install-build-deps
install-build-deps: install-uv
	...
	@echo "NOTE: Run 'source ~/.bashrc' (or open a new shell) to update your PATH."
...
deploy-local:
	...
	juju deploy $(CHARM_FILE) --resource ranger-image=$(REGISTRY)/$(ROCK_NAME):latest ranger-k8s
